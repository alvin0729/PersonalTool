...
#define kAmountMarginWidth  60
#define kScrollSpeed        40

@interface WWCalourseView ()

@property(strong,nonatomic)WWCalourseCell* view1;
@property(strong,nonatomic)WWCalourseCell* view2;
@property(strong,nonatomic)WWCalourseCell* view3;
...
-(void)setDataSource:(id<WWCalourseViewDataSource>)DataSource{
    _DataSource=DataSource;
    self.totalCount=[_DataSource WWCalourseNumber];
    
    WWCalourseCell* view1 = [[WWCalourseCell alloc]init];
    [view1 setFrame:CGRectMake(0,0, self.bounds.size.width + kAmountMarginWidth, self.bounds.size.height)];
    [_DataSource WWCalourseViewWith:view1 andIndex:0];
    view1.sectionTag=1;
    _view1=view1;
    
    
    WWCalourseCell* view2 = [[WWCalourseCell alloc]init];
    [view2 setFrame:CGRectMake(self.bounds.size.width-_margin+kAmountMarginWidth,0 , self.bounds.size.width + kAmountMarginWidth, self.bounds.size.height)];
    [_DataSource WWCalourseViewWith:view2 andIndex:1];
    view2.sectionTag=1;
    _view2=view2;
    
    
    WWCalourseCell* view3 = [[WWCalourseCell alloc]init];
    [view3 setFrame:CGRectMake(-self.bounds.size.width+_margin - kAmountMarginWidth, 0, self.bounds.size.width + kAmountMarginWidth, self.bounds.size.height)];
    [_DataSource WWCalourseViewWith:view3 andIndex:_totalCount-1];
...
-(void)StopAnimationWithTransforX:(CGFloat)transX
{
    ...
    {
        WWCalourseCell* tempView=_view2;
        _view2=_view1;
        _view1=_view3;
        _view3=tempView;
        ...
        [self.DataSource WWCalourseViewWith:_view3 andIndex:_view3.sectionTag];
        [self updateTag];
        [self animationChange];
        [self AnmiationTypeTwoChangeWithUIView:_view1 andPosition:_TempOriPostOne];
        [self AnmiationTypeTwoChangeWithUIView:_view2 andPosition:_TempOriPostTwo];
