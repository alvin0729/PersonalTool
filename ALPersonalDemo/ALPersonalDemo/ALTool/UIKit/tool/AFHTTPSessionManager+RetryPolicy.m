...
- (void)logMessage:(NSString *)message, ... {
    if (!self.retryPolicyLogMessagesEnabled) {
        return;
    }
#ifdef DEBUG
    va_list args;
...
-(void)setTasksDict:(id)tasksDict{
    objc_setAssociatedObject(self, @selector(tasksDict), tasksDict, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
-(id)tasksDict{
    return objc_getAssociatedObject(self, @selector(tasksDict));
}

-(void)setRetryDelayCalcBlock:(id)retryDelayCalcBlock{
    objc_setAssociatedObject(self, @selector(retryDelayCalcBlock), retryDelayCalcBlock, OBJC_ASSOCIATION_COPY);
}

-(id)retryDelayCalcBlock{
    return objc_getAssociatedObject(self, @selector(retryDelayCalcBlock));
}

-(void)setRetryPolicyLogMessagesEnabled:(bool)retryPolicyLogMessagesEnabled{
    objc_setAssociatedObject(self, @selector(retryPolicyLogMessagesEnabled), @(retryPolicyLogMessagesEnabled), OBJC_ASSOCIATION_ASSIGN);
}

- (bool)retryPolicyLogMessagesEnabled {
    return [(NSNumber *)objc_getAssociatedObject(self, @selector(retryPolicyLogMessagesEnabled)) boolValue];
}
