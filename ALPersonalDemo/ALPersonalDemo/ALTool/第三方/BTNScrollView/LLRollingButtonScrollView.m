...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    if ((self = [super initWithCoder:aDecoder])) {
        ...
        self.centerButtonBackgroundColor = [UIColor whiteColor];
...
//        self.notCenterButtonTextColor = [UIColor grayColor];
        self.centerButtonTextColor = [UIColor redColor];
...
{
    if (_layoutStyle == LLhorizontalLayout) {
...
//    button.titleLabel.font = self.buttonNotCenterFont;
    ...
    NSLog(@"字体哦 %@",buttonTitle);
    if ([buttonTitle isEqualToString:@"黑体"]) {
        
         button.titleLabel.font=[UIFont fontWithName:@"HYLeMiaoTiJ" size:14.0];
    }
    ...
    if ([buttonTitle isEqualToString:@"24"]) {
        
        
        button.titleLabel.font=[UIFont systemFontOfSize:18];
    }
    if ([buttonTitle isEqualToString:@"28"]) {
...
{
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(rollingScrollViewButtonIsInCenter:llRollingButtonScrollView:)]) {
        [self.llRollingButtonScrollViewDelegate rollingScrollViewButtonIsInCenter:sender llRollingButtonScrollView:self];
    }
}
...
- (void)scrollViewButtonPushed:(UIButton *)sender
{
    ...
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(rollingScrollViewButtonPushed:llRollingButtonScrollView:)]) {
        [self.llRollingButtonScrollViewDelegate rollingScrollViewButtonPushed:sender llRollingButtonScrollView:self];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(scrollViewDidScroll:)]) {
        [self.llRollingButtonScrollViewDelegate scrollViewDidScroll:scrollView];
    }
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(scrollViewWillBeginDragging:)]) {
        [self.llRollingButtonScrollViewDelegate scrollViewWillBeginDragging:scrollView];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(scrollViewWillEndDragging:withVelocity:targetContentOffset:)]) {
        [self.llRollingButtonScrollViewDelegate scrollViewWillEndDragging:scrollView withVelocity:velocity targetContentOffset:targetContentOffset];
    }
}
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
    ...
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(scrollViewDidEndDragging:willDecelerate:)]) {
        [self.llRollingButtonScrollViewDelegate scrollViewDidEndDragging:scrollView willDecelerate:decelerate];
    }
}

- (void)scrollViewWillBeginDecelerating:(UIScrollView *)scrollView
{
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(scrollViewWillBeginDecelerating:)]) {
        [self.llRollingButtonScrollViewDelegate scrollViewWillBeginDecelerating:scrollView];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(scrollViewDidEndDecelerating:)]) {
        [self.llRollingButtonScrollViewDelegate scrollViewDidEndDecelerating:scrollView];
    }
}

- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView
{
    if ([self.llRollingButtonScrollViewDelegate respondsToSelector:@selector(scrollViewDidEndScrollingAnimation:)]) {
        [self.llRollingButtonScrollViewDelegate scrollViewDidEndScrollingAnimation:scrollView];
    }
}
